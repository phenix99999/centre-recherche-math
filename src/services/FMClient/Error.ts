...
type Record<CustomFields> = {
    id: number,
    fields: CustomFields
}

export default class FMResponse<CustomFields> {
    code: number;
    count?: number;
    records?: Record<CustomFields>[];
    error?: FMError;

    constructor(xmlResponse: Response) {
        this.code = xmlResponse.code;
        ...
        if (xmlResponse.code === 401) {
            this.count = 0
            this.records = []
            return
        }
        if (xmlResponse.count === undefined) return
        if (xmlResponse.records === undefined) return

        this.count = xmlResponse.count
        this.records = xmlResponse.records.map(record => ({
            id: record.id,
            fields: (record.fields as unknown) as CustomFields
        }))
    }

    hasError() {
        return this.code !== 0 && this.code !== 401;
    }
    ...
    throwError() {
        const error = ERRORS.find(error => error.code === this.code);
        throw new Error(`Filemaker error code ${error?.code} ${error?.message}`);
    }
...
const ERRORS: FMError[] = [
    ...
    { code: 3, message: "Command is unavailable \(for example, wrong operating system or mode\)" },
    { code: 4, message: "Command is unknown" },
    { code: 5, message: "Command is invalid \(for example, a Set Field script step does not have a calculation specified\)" },
    ...
    { code: 16, message: "Operation failed; request retry \(for example, a user query\)" },
    { code: 17, message: "Attempt to convert foreign character set to UTF-16 failed" },
    { code: 18, message: "Client must provide account information to proceed" },
    { code: 19, message: "String contains characters other than A-Z, a-z, 0-9 \(ASCII\)" },
    { code: 20, message: "Command/operation canceled by triggered script" },
    { code: 21, message: "Request not supported \(for example, when creating a hard link on a file system that does not support hard links\)" },
    ...
    { code: 130, message: "Files are damaged or missing and must be reinstalled" },
    { code: 131, message: "Language pack files are missing \(such as Starter Solutions\)" },
    { code: 200, message: "Record access is denied" },
    { code: 201, message: "Field cannot be modified" },
    { code: 202, message: "Field access is denied" },
    { code: 203, message: "No records in file to print, or password doesn\'t allow print access" },
    { code: 204, message: "No access to field\(s\) in sort order" },
    { code: 205, message: "User does not have access privileges to create new records; import will overwrite existing data" },
    { code: 206, message: "User does not have password change privileges, or file is not modifiable" },
    { code: 207, message: "User does not have privileges to change database schema, or file is not modifiable" },
    ...
    { code: 212, message: "Invalid user account and/or password; please try again" },
    ...
    { code: 217, message: "User does not have sufficient privileges to modify administrator account" },
    ...
    { code: 307, message: "Transaction could not be locked because of a communication error with the host" },
    ...
    { code: 403, message: "Exceeding maximum record limit for trial version of FileMaker Pro" },
    { code: 404, message: "Sort order is invalid" },
    { code: 405, message: "Number of records specified exceeds number of records that can be omitted" },
    { code: 406, message: "Replace/reserialize criteria are invalid" },
    { code: 407, message: "One or both match fields are missing \(invalid relationship\)" },
    { code: 408, message: "Specified field has inappropriate data type for this operation" },
    { code: 409, message: "Import order is invalid" },
    { code: 410, message: "Export order is invalid" },
    { code: 412, message: "Wrong version of FileMaker Pro used to recover file" },
    { code: 413, message: "Specified field has inappropriate field type" },
    { code: 414, message: "Layout cannot display the result" },
    { code: 415, message: "One or more required related records are not available" },
    { code: 416, message: "A primary key is required from the data source table" },
    ...
    { code: 502, message: "Number value does not meet validation entry options" },
    { code: 503, message: "Value in field is not within the range specified in validation entry options" },
    { code: 504, message: "Value in field is not unique, as required in validation entry options" },
    { code: 505, message: "Value in field is not an existing value in the file, as required in validation entry options" },
    { code: 506, message: "Value in field is not listed in the value list specified in validation entry option" },
    ...
    { code: 513, message: "No validation was specified but data cannot fit into the field" },
    { code: 600, message: "Print error has occurred" },
    { code: 601, message: "Combined header and footer exceed one page" },
    { code: 602, message: "Body doesn\'t fit on a page for current column setup" },
    ...
    { code: 708, message: "Can\'t import the file, or need color monitor support to import file" },
    { code: 711, message: "Import translator cannot be found" },
    { code: 714, message: "Password privileges do not allow the operation" },
    { code: 715, message: "Specified Excel worksheet or named range is missing" },
    { code: 716, message: "A SQL query using DELETE, INSERT, or UPDATE is not allowed for ODBC import" },
    { code: 717, message: "There is not enough XML/XSL information to proceed with the import or export" },
    { code: 718, message: "Error in parsing XML file \(from Xerces\)" },
    { code: 719, message: "Error in transforming XML using XSL \(from Xalan\)" },
    { code: 720, message: "Error when exporting; intended format does not support repeating fields" },
    { code: 721, message: "Unknown error occurred in the parser or the transformer" },
    { code: 722, message: "Cannot import data into a file that has no fields" },
    { code: 723, message: "You do not have permission to add records to or modify records in the target table" },
    { code: 724, message: "You do not have permission to add records to the target table" },
    { code: 725, message: "You do not have permission to modify records in the target table" },
    { code: 726, message: "Source file has more records than the target table; not all records were imported" },
    { code: 727, message: "Target table has more records than the source file; not all records were updated" },
    { code: 729, message: "Errors occurred during import; records could not be imported" },
    { code: 730, message: "Unsupported Excel version; convert file to the current Excel format and try again" },
    { code: 731, message: "File you are importing from contains no data" },
    { code: 732, message: "This file cannot be inserted because it contains other files" },
    { code: 733, message: "A table cannot be imported into itself" },
    { code: 734, message: "This file type cannot be displayed as a picture" },
    { code: 735, message: "This file type cannot be displayed as a picture; it will be inserted and displayed as a file" },
    { code: 736, message: "Too much data to export to this format; data will be truncated" },
    ...
    { code: 804, message: "File cannot be opened as read-only in its current state" },
    { code: 805, message: "File is damaged; use Recover command" },
    { code: 806, message: "File cannot be opened with this version of FileMaker Pro" },
    { code: 807, message: "File is not a FileMaker Pro file or is severely damaged" },
    { code: 808, message: "Cannot open file because access privileges are damaged" },
    { code: 809, message: "Disk/volume is full" },
    { code: 810, message: "Disk/volume is locked" },
    { code: 811, message: "Temporary file cannot be opened as FileMaker Pro file" },
    { code: 812, message: "Exceeded host’s capacity" },
    { code: 813, message: "Record synchronization error on network" },
    { code: 814, message: "File\(s\) cannot be opened because maximum number is open" },
    { code: 815, message: "Couldn’t open lookup file" },
    { code: 816, message: "Unable to convert file" },
    { code: 817, message: "Unable to open file because it does not belong to this solution" },
    ...
    { code: 823, message: "Cannot set file to single-user; guests are connected" },
    { code: 824, message: "File is damaged or not a FileMaker file" },
    { code: 825, message: "File is not authorized to reference the protected file" },
    { code: 826, message: "File path specified is not a valid file path" },
    { code: 827, message: "File was not created because the source contained no data or is a reference" },
    ...
    { code: 906, message: "Current file is not shared; command can be used only if the file is shared" },
    ...
    { code: 1204, message: "Number, text constant, field name, or \"\(\" expected" },
    ...
    { code: 1208, message: "Operator missing, function not found, or \"\(\" not expected" },
    { code: 1209, message: "Name \(such as field name or layout name\) is missing" },
    ...
    { code: 1212, message: "An operator \(for example, +, -, *\) is expected here" },
    { code: 1213, message: "This variable has already been defined in the Let function" },
    { code: 1214, message: "AVERAGE, COUNT, EXTEND, GETREPETITION, MAX, MIN, NPV, STDEV, SUM, and GETSUMMARY: expression found where a field alone is needed" },
    { code: 1215, message: "This parameter is an invalid Get function parameter" },
    { code: 1216, message: "Only summary fields are allowed as first argument in GETSUMMARY" },
    ...
    { code: 1221, message: "Data type must be number, date, time, or timestamp" },
    ...
    { code: 1225, message: "Function referred to is not supported in this context" },
    { code: 1300, message: "The specified name can\'t be used" },
    { code: 1301, message: "A parameter of the imported or pasted function has the same name as a function in the file" },
    { code: 1400, message: "ODBC client driver initialization failed; make sure ODBC client drivers are properly installed" },
    ...
    { code: 1504, message: "SMTP server requires the connection to be encrypted" },
    { code: 1505, message: "Specified authentication is not supported by SMTP server" },
    { code: 1506, message: "Email message\(s\) could not be sent successfully" },
    { code: 1507, message: "Unable to log in to the SMTP server" },
    { code: 1550, message: "Cannot load the plug-in, or the plug-in is not a valid plug-in" },
    { code: 1551, message: "Cannot install the plug-in; cannot delete an existing plug-in or write to the folder or disk" },
    ...
    { code: 1629, message: "Connection timed out; the timeout value is 60 seconds" },
    { code: 1630, message: "URL format is incorrect" },
    { code: 1631, message: "Connection failed" },
    { code: 1632, message: "Certificate cannot be authenticated by a supported certificate authority" },
    { code: 1633, message: "Certificate is valid but still causes an error \(for example, the certificate has expired\)" },
]    
